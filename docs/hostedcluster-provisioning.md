...
Your AWS account must be registered with the platform before you can create clusters. Ask `@rrp-team-ic` in `#forum-rosa-hyperfleet` to allowlist your account — provide your **AWS account ID** and the target **environment** (e.g. integration). This is a one-time step per account per environment.
...
- For assistance, reach out to @rrp-team-ic in #forum-rosa-hyperfleet.
