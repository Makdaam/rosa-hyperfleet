...
A kube-applier controller runs on each Management Cluster, reading desire documents from DynamoDB tables (written by the hyperfleet-operator in the RC account) via GSI two-speed polling and applying them to the local Kubernetes API. If the MC API is non-responsive, observability alerts notify SREs.
