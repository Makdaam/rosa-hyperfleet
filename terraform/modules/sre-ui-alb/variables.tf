...
variable "oidc_client_id" {
  description = "OIDC client ID registered on the IdP. Required when oidc_enabled = true."
  type        = string
  default     = ""
}

variable "oidc_client_secret" {
  description = "OIDC client secret. Passed to the ALB at apply time and stored encrypted by the ALB service — never persisted in Terraform state in plain text."
  type        = string
  default     = ""
  sensitive   = true
}
