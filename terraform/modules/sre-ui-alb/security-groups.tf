...
# - Egress to node SG on service ports: 80 (Grafana), 443 (ArgoCD), 9090 (Prometheus/Thanos), 3100 (Loki)
#
...
# Egress: to Grafana pods (port 80)
resource "aws_vpc_security_group_egress_rule" "alb_to_http_services" {
  security_group_id            = aws_security_group.alb.id
  description                  = "Allow traffic to Grafana pods (port 80)"
  ip_protocol                  = "tcp"
  from_port                    = 80
  to_port                      = 80
...
# Egress: to ArgoCD pods (port 443)
resource "aws_vpc_security_group_egress_rule" "alb_to_argocd" {
  security_group_id            = aws_security_group.alb.id
  description                  = "Allow traffic to ArgoCD pods (port 443)"
  ip_protocol                  = "tcp"
  from_port                    = 443
  to_port                      = 443
  referenced_security_group_id = var.node_security_group_id
...
resource "aws_vpc_security_group_ingress_rule" "nodes_from_alb_http" {
  ...
  description                  = "Allow SRE ALB traffic to Grafana pods (port 80)"
  ip_protocol                  = "tcp"
  from_port                    = 80
  to_port                      = 80
...
resource "aws_vpc_security_group_ingress_rule" "nodes_from_alb_argocd" {
  security_group_id            = var.node_security_group_id
  description                  = "Allow SRE ALB traffic to ArgoCD pods (port 443)"
  ip_protocol                  = "tcp"
  from_port                    = 443
  to_port                      = 443
  referenced_security_group_id = aws_security_group.alb.id
